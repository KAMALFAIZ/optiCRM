...
export const leadsApi = {
  ...
  convert: async (id: string, data: ConvertLeadRequest): Promise<void> => {
    await apiClient.post(`/leads/${id}/convert`, data);
  },

  getScore: async (leadId: string): Promise<LeadScore> => {
    const response = await apiClient.get<ApiResponse<LeadScore>>(`/leads/${leadId}/score`);
    return response.data.data!;
  },

  refreshScore: async (leadId: string): Promise<LeadScore> => {
    const response = await apiClient.post<ApiResponse<LeadScore>>(`/leads/${leadId}/score/refresh`);
    return response.data.data!;
  },
};
