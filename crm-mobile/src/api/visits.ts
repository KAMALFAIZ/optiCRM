...
export const visitsApi = {
  ...
  checkIn: async (id: string, latitude?: number, longitude?: number): Promise<Visit> => {
    const response = await apiClient.patch<ApiResponse<Visit>>(`/visits/${id}/check-in`, null, {
  ...
  checkOut: async (id: string, notes?: string, outcome?: string): Promise<Visit> => {
    const response = await apiClient.patch<ApiResponse<Visit>>(`/visits/${id}/check-out`, null, {
      params: { notes, outcome },
    });
    return response.data.data!;
  },
};
