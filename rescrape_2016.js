/**
 * Re-scraper Elections Legislatives Maroc 2016
 * Uses DNS (www.elections.ma) which works, not the hardcoded IP
 */
...
function apiCall(regionCode, provinceCode, circCode, retries = 4) {
    ...
    const attempt = (n) => {
      ...
      const req = https.request(opts, (res) => {
        if (res.statusCode === 503) {
          const wait = (n + 1) * 5000;
          console.log('  503 - retry in ' + (wait/1000) + 's');
...
async function main() {
  const allData = [];
  const progressFile = path.join(OUT_DIR, 'rescrape_progress_2016.json');
  let startIdx = 0;
  ...
  if (fs.existsSync(progressFile)) {
    try {
      ...
      console.log('Resumed from index ' + startIdx + ', ' + allData.length + ' rows loaded');
    } catch(e) { console.log('Progress file error, starting fresh'); }
  }

  console.log('Starting scrape of ' + CIRCUITS.length + ' circuits...\n');
  ...
  for (let i = startIdx; i < CIRCUITS.length; i++) {
    ...
    const type = isReg ? 'وطنية' : 'محلية';

    console.log('[' + (i+1) + '/' + CIRCUITS.length + '] ' + daireName + ' (' + provinceName + ')');
    ...
    try {
      ...
      fs.writeFileSync(tmpFile, JSON.stringify({ nextIdx: i + 1, rows: allData }));
      fs.renameSync(tmpFile, progressFile);
    } catch(e) {
      console.log('  progress save warning: ' + e.message);
    }

    if (i < CIRCUITS.length - 1) await delay(DELAY);
  }
  ...
  const csvRows = allData.map(r =>
    fields.map(f => '"' + String(r[f] != null ? r[f] : '').replace(/"/g, '""') + '"').join(',')
  );
  const csvPath = path.join(OUT_DIR, 'elections_2016.csv');
  fs.writeFileSync(csvPath, '﻿' + header + '\n' + csvRows.join('\n'), 'utf8');
  console.log('\nCSV: ' + csvPath + ' (' + allData.length + ' rows)');
